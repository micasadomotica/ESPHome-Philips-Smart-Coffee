    ...
    namespace philips_coffee_machine
    {
        namespace philips_power_switch
        {
            ...
            void Power::loop()
            {
                ...
                // Start a new power trip (non-blocking, two phases)
                if (!power_trip_active_ && now - last_power_trip_ > power_trip_delay_ + POWER_TRIP_RETRY_DELAY)
                {
                    ...
                    ESP_LOGD(TAG, "Power trip %d: cutting display power", power_trip_count_ + 1);
                    power_pin_->digital_write(!(*initial_state_));
                    power_trip_active_ = true;
                    power_trip_start_time_ = now;
                }

                // Finish the ongoing power trip after power_trip_delay_
                if (power_trip_active_ && now - power_trip_start_time_ >= power_trip_delay_)
                {
                    ...
                    power_trip_count_++;
                    ESP_LOGD(TAG, "Power trip %d complete: display power restored", power_trip_count_);
                }
            }
            ...
            void Power::write_state(bool state)
            {
                if (state)
                {
                    ...
                    // Wake the display using power trips, performed non-blocking in loop()
                ...
                // The state will be published once the display starts/stops sending messages
            }
            ...
            void Power::update_state(bool state)
            {
                if (this->state != state)
                {
                    ...
                    if (state && should_power_trip_)
                    {
                        ...
                        // If we were mid-trip, make sure display power is restored
                        if (power_trip_active_)
                        {
                            power_pin_->digital_write(*initial_state_);
                            power_trip_active_ = false;
                        }
